...
from domain.Reaccionables.Comentariao import Comentario
...
class MyTestCase(unittest.TestCase):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.puntuacion = Puntuacion(True, None, None, self.jorge)
    ...
    def test_reaccionar(self):
        self.jorge.rasgos[0].serReaccionado(self.reaccionDeJorge)
    ...
    def test_rasgo_reaccionar_y_cambiar_reaccion(self):
        self.jorge.rasgos[0].serReaccionado(self.reaccionDeJorge)
        self.jorge.rasgos[0].serReaccionado(self.reaccionEnojada)
        self.assertEqual("Me Enoja", self.jorge.rasgos[0].reacciones[0].tipo.texto)

    def test_rasgo_reaccionar_y_sacar_reaccion(self):
        self.jorge.rasgos[0].serReaccionado(self.reaccionDeJorge)
        self.jorge.rasgos[0].sacarReaccion(self.reaccionDeJorge)
        self.assertEqual(len(self.jorge.rasgos[0].reacciones), 0)

    def test_rasgo_validable_puntuar(self):
        self.tobias.rasgos[0].expertise.serPuntuado(self.puntuacion)
        self.assertEqual(True, self.tobias.rasgos[0].expertise.puntuaciones[0].esPositiva)

    def test_rasgo_validable_puntuar_y_sacar_puntuacion(self):
        self.tobias.rasgos[0].expertise.serPuntuado(self.puntuacion)
        self.tobias.rasgos[0].expertise.serDespuntuado(self.puntuacion)
        self.assertEqual(0, len(self.tobias.rasgos[0].expertise.puntuaciones))

    def test_rasgo_validable_puntuar_varias_veces(self):
        for i in range(5):
            self.tobias.rasgos[0].expertise.serPuntuado(self.puntuacion)
        self.assertEqual(1, len(self.tobias.rasgos[0].expertise.puntuaciones))

    def test_comentar_puntuacion(self):
        self.tobias.rasgos[0].expertise.serPuntuado(self.puntuacion)
        self.tobias.rasgos[0].expertise.puntuaciones[0].comentar(Comentario(datetime.now(), "Sos un groso"))
        self.tobias.rasgos[0].expertise.puntuaciones[0].responder(Comentario(datetime.now(), "Gracias capo"))
        self.assertEqual("Sos un groso",  self.tobias.rasgos[0].expertise.puntuaciones[0].comentario.texto)
        self.assertEqual("Gracias capo", self.tobias.rasgos[0].expertise.puntuaciones[0].respuesta.texto)
